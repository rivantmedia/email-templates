function initiatePopulateCalendar() {
	populateData(actualCalendarSheetName);
	populateData(allocatedCalendarSheetName);
}
...
function activateInitiatePopulateCalendar() {
	ScriptApp.newTrigger("initiatePopulateCalendar").forSpreadsheet(SpreadsheetApp.getActiveSpreadsheet()).onChange().create();

	ScriptApp.newTrigger("initiatePopulateCalendar").timeBased().everyDays(1).atHour(0).create();
}
